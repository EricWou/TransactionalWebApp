...
//when putting "import type" in front of imports you mark them as "type"                                                                      
//meaning you are letting TypeScript know that they can be removed from                                                                       
//the output(aka only needed at build time.)


type ConnectionStatus = {                     
  isConnected: boolean;         
};                              
//this "type" on the other hand is called "Type alias"
//this allows defining types as custom names to primitives or objects. (code reusability) 
...
we recommend using getStaticProps. 
...
function TeamMember({member}){
    ...
    if(members.some(e => e.name === member.name && e.id === member.id)){       //(<object> => <object>.<property> === parameter)
      return <p>{member.name} is a team member.</p>
    }
      return <p>{member.name} is not a team member.</p>
    //can't compare directly 2 objects in React
}

let testObject = {name: "Sonic", id:666};  
//let testObject = {name:"Eric", id:301};  
//let testObject = {notname: "Eric", id:666};  
//let testObject = {id:666, name: "Eric"};

export default function Home({ isConnected, } : InferGetStaticPropsType<typeof getStaticProps>) {
  //const [inputPassword, setTextString] = useState(null);
  const inputUsername = useRef(null);
  const inputPassword = useRef(null);
  const [message, setMessage] = useState('');
  
  let members =[
    {name:"Eric", id:301, password: "chinaman"},
    {name:"Vanessa", id:302, password: "vivaperu"},
    {name:"Rian", id:303, password: "pinoy4life"}
  ];

  function ReceivePassword(){
    return <div>
              <label>Enter username</label>
              <input ref={inputUsername} id="inputUsername"/>
              <label><br/>Enter password</label>
              <input ref={inputPassword} id="inputPassword"/>
              <button id="signIn" type="button" onClick={ComparePassword}>Sign in</button>
              <p>{message}</p>
            </div>
}

function ComparePassword(){
    const compareUsername = inputUsername.current.value;
    const comparePassword = inputPassword.current.value;
    const memberFound = members.find(e => e.name === compareUsername);
    const passwordCorrect = members.find(e => e.password === comparePassword);
    
    if (memberFound) {
      if (passwordCorrect) {
        setMessage(`Welcome ${memberFound.name}`);
      }
    }
    else {
